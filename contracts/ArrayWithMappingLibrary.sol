...
library ArrayWithMappingLibrary {
  ...
  function _arrayContains(
    ...
    bytes memory encodedElementIndex = IERC725Y(_UNIVERSAL_PROFILE).getData(
      bytes32(bytes.concat(
        arrayElementMapPrefix, bytes2(0), bytes20(arrayElement)
      ))
    );    
  ...
  function _addElement(
    ...
    uint256 oldArrayLength = uint256(bytes32(IERC725Y(_UNIVERSAL_PROFILE).getData(arrayLengthKey)));
    uint256 newArrayLength = oldArrayLength + 1;
    (bool response, uint256 elementIndex) = _arrayContains(_UNIVERSAL_PROFILE, arrayElementMapPrefix, arrayElement);
    delete elementIndex;
  ...
  function _removeElement(
    ...
    bytes memory encodedArrayLastElement = IERC725Y(_UNIVERSAL_PROFILE).getData(bytes32(bytes.concat(arrayIndexPerfix, bytes16(uint128(newArrayLength)))));
    ...
    keys[1] = bytes32(bytes.concat(arrayIndexPerfix, bytes16(uint128(elementIndex))));
