...
function _MULTISIG_PROPOSAL_SIGNATURE(bytes6 _proposalIdentifier)
    pure
    returns (bytes10 KEY)
{
    KEY = bytes10(
        bytes.concat(_MULTISIG_PROPOSAL_KEY_BYTES4_PREFIX, _proposalIdentifier)
    );
}
...
// --> abi.encode(bytes[])
function _MULTISIG_PROPOSAL_PAYLOADS_KEY(bytes10 _proposalSignature)
    pure
    returns (bytes32 KEY)
{
    KEY = bytes32(bytes.concat(_proposalSignature, _MULTISIG_PAYLOADS_SUFFIX));
}
