...
// LSP6 Constants
import {NoPermissionsSet, NotAuthorised} from "@lukso/lsp-smart-contracts/contracts/LSP6KeyManager/LSP6Errors.sol";
import {setDataSingleSelector, setDataMultipleSelector} from "@lukso/lsp-smart-contracts/contracts/LSP6KeyManager/LSP6Constants.sol";

// Multisig Constants
import {_LSP6KEY_ADDRESSPERMISSIONS_MULTISIGPERMISSIONS_PREFIX, _PERMISSION_VOTE, _PERMISSION_PROPOSE, _PERMISSION_ADD_PERMISSION, _PERMISSION_REMOVE_PERMISSION, _PERMISSION_EXECUTE_PROPOSAL, _MULTISIG_QUORUM_KEY, _MULTISIG_PARTICIPANTS_ARRAY_KEY, _MULTISIG_PARTICIPANTS_ARRAY_PREFIX, _MULTISIG_PARTICIPANTS_MAPPING_PREFIX, _MULTISIG_PROPOSAL_SIGNATURE, _MULTISIG_PROPOSAL_PAYLOADS_KEY} from "./MultisigConstants.sol";
...
contract MultisigKeyManager is IMultisigKeyManager {
    ...
    constructor(address payable _UNIVERSAL_PROFILE, address _KEY_MANAGER) {
    ...
    ) public view override returns (bytes32 _hash) {
        _hash = keccak256(
            abi.encode(
                address(this),
                _from,
    ...
    function claimPermission(
        ...
        bytes32 _hash = getNewPermissionHash(_from, msg.sender, _permissions)
     ...
     * @inheritdoc IMultisigKeyManager
     */
    function addPermissions(address _to, bytes32 _permissions)
        external
        override
    {
        _verifyPermission(
     ...
     * @inheritdoc IMultisigKeyManager
     */
    function removePermissions(address _to, bytes32 _permissions)
        external
        override
    {
        _verifyPermission(
    ...
    function proposeExecution(
        ...
        ILSP6KeyManager(KEY_MANAGER).execute(
            abi.encodeWithSelector(setDataSingleSelector, key, value)
        );
    ...
    function execute(
        ...
        if (_signatures.length != _signers.length)
            revert IndexedError("Multisig", 0x03);

        bytes32[] memory keys = new bytes32[](2);
        keys[0] = _MULTISIG_PARTICIPANTS_ARRAY_KEY;
        keys[1] = _MULTISIG_QUORUM_KEY;
        bytes[] memory encodedResponses = IERC725Y(UNIVERSAL_PROFILE).getData(
            keys
        );
        uint256 votingMembers = uint256(bytes32(encodedResponses[0]));
        uint8 quorum = uint8(bytes1(encodedResponses[1]));

        if ((_signatures.length * 100) / votingMembers < quorum)
            revert IndexedError("Multisig", 0x04);
        ...
        if ((positiveResponses * 100) / votingMembers > quorum) {
            bytes[] memory _payloads = abi.decode(
                IERC725Y(UNIVERSAL_PROFILE).getData(
                    _MULTISIG_PROPOSAL_PAYLOADS_KEY(_proposalSignature)
                ),
                (bytes[])
            );
     ...
     */
    function _getPermissions(address _from)
        internal
        view
    ...
    function _verifyPermission(
        ...
        if (permissions & _permission == 0)
            revert NotAuthorised(_from, _permissionName);
    }
